 ...
 */
public class DepthPeelingStencil {
    ...
    private int progQuadFlat = 0;

    public DepthPeelingStencil(int width, int height, float sXY, float zMinWorld, float zMaxWorld) {
    ...
    public void initGL() {
        ...
        progQuadFlat   = compileProgram(VS_QUAD, FS_FLAT); 
    ...
    public void renderSlice(float z_s) {
        ...
        // ---------------- Pass A: Stencil parity (even/odd) under z_s ----------------
        ...
        // Toggle le bit de stencil à chaque fragment accepté
        ...
        glUniform1i(glGetUniformLocation(progMeshDiscardAbove, "uMode"), 0); // 0 => garder z <= z_s+eps
        ...
        glDisable(GL_BLEND);
        glStencilMask(0xFF);*/
        // ---------------- Pass D: Compose using stencil == 1 (inside) ----------------
        glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, colorTex, 0);
        IntBuffer db = BufferUtils.createIntBuffer(1); db.put(GL_COLOR_ATTACHMENT0).flip();
        glDrawBuffers(db);
        ...
        // On ne dessine que là où la parité est 1 (intérieur)
        glEnable(GL_STENCIL_TEST);
        glStencilFunc(GL_EQUAL, 0x1, 0xFF);
        glStencilOp(GL_KEEP, GL_KEEP, GL_KEEP);

        // --- Stencil-only (couleur pleine) ---
        // Si tu veux juste vérifier le masque, garde ce chemin :
        glUseProgram(progQuadFlat); // VS_QUAD + FS_FLAT
        glUniform4f(glGetUniformLocation(progQuadFlat, "uColor"), 1f, 0f, 0f, 1f);
        glBindVertexArray(vaoQuad);
        glDrawArrays(GL_TRIANGLES, 0, 3);
        glBindVertexArray(0);

        // Si tu veux le gradient distance, commente le bloc ci-dessus et décommente ci-dessous:
        /*
    ...
    public void renderSlice2(float z_s) {
    glBindFramebuffer(GL_FRAMEBUFFER, fbo);
    ...
    private static final String FS_FLAT = "#version 330 core\n" +
    "uniform vec4 uColor;\n" +
    "out vec4 oColor;\n" +
